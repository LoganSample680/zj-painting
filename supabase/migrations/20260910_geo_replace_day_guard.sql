-- geo_replace_day, revision 4: the past is read-only, and no tape means no
-- sweep.
--
-- Owner 2026-09-04, after the deriver settled: "now that were done fucking
-- with the logic, its sound, just need to protect the logic from overwrites
-- by new devices or shared devices" and "cant risk data going away ever".
--
-- Two guards, both here because this is the one writer of automatic rows
-- (CLAUDE.md 17) and the only place a device cannot route around:
--
--   1. A day whose window closed more than fourteen days ago is refused,
--      quietly. Nobody in a two-truck shop approves a time card, so the lock
--      is automatic. Hand edits do not come through here and still work.
--   2. p_sweep. The phone passes false when it derived a day without its own
--      motion history (a laptop, a new phone, a shared iPad before this
--      person's claim on it). Rows may then be added or refreshed, never
--      retired: a derive cannot delete what it has no evidence about.
--
-- The eight-argument signature is dropped rather than left beside the new
-- one: PostgREST resolves the call by name and the two would be ambiguous
-- for every existing caller, which passes nothing for p_sweep and expects
-- the default.

drop function if exists geo_replace_day(uuid, uuid, text, timestamptz, timestamptz, jsonb, jsonb, jsonb);
